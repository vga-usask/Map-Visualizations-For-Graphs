...
namespace RPGraph
{
    ...
    : root_center{root_center}, root_length{root_length}, theta{theta}
    {
        ...
        // Ensure the RPCommon singleton is initialized and node polygons are read
    }
...
Real2DVector BarnesHutApproximator::approximateForce(Coordinate particle_pos, float particle_mass, float theta) {
    ...
    {
        std::queue<BarnesHutCell*> local_cells_to_check;
        Real2DVector local_force = Real2DVector(0.0, 0.0);
...

        {
            while (!cells_to_check.empty()) {
                ...
                {
                    local_cells_to_check.push(cur_cell);

                    while (!local_cells_to_check.empty()) {
                        cur_cell = local_cells_to_check.front();
                        local_cells_to_check.pop();

                        const float D2 = distance2(particle_pos, cur_cell->mass_center);
                        if (D2 == 0) {
                            if (cur_cell->num_subparticles == 0) continue;
                            else {
                                local_force += Real2DVector(rand(), rand());
                                continue;
                            }
                        }

                        if (cur_cell->length * cur_cell->length / D2 < theta * theta || cur_cell->num_subparticles == 0) {
                            local_force += direction(particle_pos, cur_cell->mass_center) *
                                           (particle_mass * cur_cell->total_mass / D2);
                        } else {
                            for (int i = 0; i < 4; ++i) {
                                if (cur_cell->sub_cells[i] != nullptr) {
                                    local_cells_to_check.push(cur_cell->sub_cells[i]);
                                }
                            }
                        }
                    }
