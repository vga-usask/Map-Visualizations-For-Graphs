...
def main():
    ...
    for i in tqdm(range(1, len(cluster_id_to_nodes_obj) + 1)):
        ...
        x_scale_factor= float(H.nodes[cluster_id]['width']) * 20 / width if width>0 else 1
        y_scale_factor= float(H.nodes[cluster_id]['height']) * 20 / height if height>0 else 1
    ...
    os.system(f'gvmap -e  -c 1 -r 500 {G_OUT_PATH} > {DRAWING_DOT_PATH}')
